...
export function seedDatabaseIfEmpty() {
  ...
  if (count.count > 0) {
    console.log(`Database already has ${count.count} activities, skipping seed`);
    return;
  }

  console.log('Database is empty, seeding with default activities...');
  ...
  insertMany(SEED_ACTIVITIES);
  
  console.log(`✅ Seeded ${SEED_ACTIVITIES.length} activities (${SEED_ACTIVITIES.filter((a: any) => a.type === 'retro').length} retros + ${SEED_ACTIVITIES.filter((a: any) => a.type === 'icebreaker').length} icebreakers)`);
}
